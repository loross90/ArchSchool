...
async def read_presentation(title: str):
    # get presentation
    responsePresentation = requests.get(
        "http://service_presentation:8082/presentations/"+title)
    print(responsePresentation)
    if responsePresentation is None:
        raise HTTPException(
            status_code=404, detail="No presentations for this title")
    ...
    # get Author by id
    responseAuthor = requests.get(
        "http://service_author:8081/authors/"+str(presentation.author_id))
    print(responseAuthor)
    if responseAuthor is None:
    ...
    presentationWithAuthor = Presentation_With_Author(
        presentation.title, presentation.date, author)
